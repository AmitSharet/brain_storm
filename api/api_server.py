...
def run_api_server(host='127.0.0.1', port=5000, database_url='mongodb://0.0.0.0:27017/'):
    ...
    def getUsers():
        d = {}
        print(list(db.users.find(d, {'_id': 0})))
        print(type(list(db.users.find(d, {'_id': 0}))))
        return json.dumps(list((db.users.find(d, {'_id': 0}))), default=json_util.default)
    ...
    def getUserSnapshots(user_id):
        ...
        return json.dumps(list(db.snapshots.find({'userId': str(user_id)}, {'_id': 0})), default=json_util.default)
    ...
    def getUserSnapshot(user_id, snapshot_id):
        ...
        return json.dumps(snapshot, default=json_util.default)
